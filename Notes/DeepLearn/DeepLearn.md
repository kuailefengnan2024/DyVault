...
| **模型** | **输入**                                                                 | **输出**                                                                 |
|----------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|
| **CLIP** | - **图像**：像素值张量，形状为 `(batch_size, channels, height, width)`，例如 `(1, 3, 224, 224)`（RGB 图像，224x224）。<br>- **文本**：文本字符串，分词后转换为 token ID 序列，形状为 `(batch_size, sequence_length)`，例如 `(1, 77)`（最大长度 77）。 | - **图像嵌入**：固定长度的向量，形状为 `(batch_size, embedding_dim)`，例如 `(1, 512)`（512 维）。<br>- **文本嵌入**：固定长度的向量，形状为 `(batch_size, embedding_dim)`，例如 `(1, 512)`。<br>- **相似度**：图像和文本嵌入的相似度分数（例如余弦相似度），用于跨模态任务。 |
| **VAE**  | - **训练阶段**：原始数据 \( x \)，例如图像张量，形状为 `(batch_size, channels, height, width)`，如 `(batch_size, 1, 28, 28)`（MNIST 灰度图像）。<br>- **推理/生成阶段**：潜在变量 \( z \)，从标准正态分布采样，形状为 `(batch_size, latent_dim)`，例如 `(batch_size, 32)`。 | - **训练阶段**：<br>  - 编码器输出：均值 \( \mu \) 和方差 \( \sigma \)，形状均为 `(batch_size, latent_dim)`，例如 `(batch_size, 32)`。<br>  - 潜在变量 \( z \)，形状为 `(batch_size, latent_dim)`。<br>  - 解码器输出：重建数据 \( \hat{x} \)，形状与输入相同，例如 `(batch_size, 1, 28, 28)`。<br>- **推理/生成阶段**：生成数据 \( \hat{x} \)，形状与训练数据相同，例如 `(batch_size, 1, 28, 28)`。 |
