...
### Encoder流程（维度与流程融合）
输入为单通道灰度图像 \([1, 28, 28]\)（通道数×高度×宽度）：

1. **卷积层1**：输入 \([1, 28, 28]\) → 32个3×3卷积核，步幅1，填充0，ReLU激活 → 输出 \([32, 26, 26]\)。  
2. **池化层1**：输入 \([32, 26, 26]\) → 2×2最大池化，步幅2 → 输出 \([32, 13, 13]\)。  
3. **卷积层2**：输入 \([32, 13, 13]\) → 64个3×3卷积核，步幅1，填充0，ReLU激活 → 输出 \([64, 11, 11]\)。  
4. **池化层2**：输入 \([64, 11, 11]\) → 2×2最大池化，步幅2 → 输出 \([64, 5, 5]\)。  
5. **展平**：输入 \([64, 5, 5]\) → 展平为1维 → 输出 \([1600]\)。  
6. **全连接层1**：输入 \([1600]\) → 256个神经元，ReLU激活 → 输出 \([256]\)。  
7. **全连接层2（均值分支）**：输入 \([256]\) → 128个神经元，无激活 → 输出 \(\mu\) \([128]\)。  
8. **全连接层2（方差分支）**：输入 \([256]\) → 128个神经元，无激活 → 输出 \(\log(\sigma^2)\) \([128]\)，计算 \(\sigma = \exp(0.5 \cdot \log(\sigma^2))\) \([128]\)。  
9. **重参数化**：输入 \(\mu\) \([128]\) 和 \(\sigma\) \([128]\) → \( z = \mu + \sigma \cdot \epsilon \)（\(\epsilon \sim \mathcal{N}(0, 1)\)，\([128]\)） → 输出 \(z\) \([128]\)。
